...
public class MainPanel extends Panel implements MouseListener{ 
	...
	private static boolean isBlack = true;  //标记是黑子
	@SuppressWarnings("unused")
	private static int click_X;  //鼠标点击位置的x坐标
	@SuppressWarnings("unused")
	private static int click_Y;  //鼠标点击位置的y坐标
	private char[][] allChess= new char[ROW][COLUMN]; //棋盘二维数组
	public MainPanel(){ 
		super();  
	...
	public void paint(Graphics g){ 
		for(int i=0;i<ROW;i++){//划棋盘的所有横线
			g.setColor(Color.BLACK); 
			g.drawLine(20, 20+i*GAP, 640-20, 20+i*GAP);     
		}
		for(int i=0;i<COLUMN;i++){//划棋盘的所有纵线
			g.setColor(Color.BLACK); 
			g.drawLine(20+i*GAP, 20, 20+i*GAP, 640-20); 
		}
		for(int i=0;i<allChess.length;i++){ //画棋子
			for(int j=0;j<allChess[i].length;j++){ 
				if(allChess[i][j]=='~'){ 
